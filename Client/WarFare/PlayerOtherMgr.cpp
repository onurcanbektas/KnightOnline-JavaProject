...
void CPlayerOtherMgr::Tick(const __Vector3& vPosPlayer)
{
	...
	for(; it2 != itEnd2; )
	{
		...
		else if (*pNPC->m_InfoBase.eNation==NationEnum::KARUS || *pNPC->m_InfoBase.eNation==NationEnum::ELMORAD)
		{
...
CPlayerNPC*	CPlayerOtherMgr::CharacterGetByNearstEnemy(NationEnum & eNation, const __Vector3& vPosPlayer) // 가장 가까운 적 가져오기..
{
	...
	for(; it != itEnd; it++)
	{
		...
		if (!ACT_WORLD->canAttackSameNation() && (pNPC->m_InfoBase.eNation == eNation))
			return false;
		if (!ACT_WORLD->canAttackOtherNation() && (*eNation == NationEnum::ELMORAD && pNPC->m_InfoBase.eNation == NationEnum::KARUS))
			return false;
		if (!ACT_WORLD->canAttackOtherNation() && (*eNation == NationEnum::KARUS && pNPC->m_InfoBase.eNation == NationEnum::ELMORAD))
			return false;
		*/
		//-------------------------------------------------------------------------
