...
void CGameProcMain::Init()
{
	...
	*/
	...
	for( int i = /*IDS_CMD_WHISPER*/6200; i <= /*IDS_CMD_GAME_SAVE*/6225; i++ ) //명령어 로딩...
	{
		s_szCmdMsg[i - /*IDS_CMD_WHISPER*/6200] = szTemp[i - /*IDS_CMD_WHISPER*/6200];
	...
	{
		int iIDSndBattle = ((NationEnum::KARUS == *s_pPlayer->m_InfoBase.eNation) ? ID_SOUND_BGM_KA_BATTLE : ID_SOUND_BGM_EL_BATTLE);
...
bool CGameProcMain::MsgRecv_MyInfo_All(DataPack* pDataPack, int& iOffset)
{
	...
	s_pPlayer->m_InfoBase.eNation = &NationEnum::forValue(CAPISocket::Parse_GetByte(pDataPack->m_pData, iOffset));
...
bool CGameProcMain::MsgRecv_Regen(DataPack* pDataPack, int& iOffset)
{
	...
	if(s_pPlayer->Nation()==NationEnum::KARUS) CGameProcedure::s_pFX->TriggerBundle(s_pPlayer->IDNumber(), -1, FXID_REGEN_KARUS, s_pPlayer->IDNumber(), -1);
	else if(s_pPlayer->Nation()==NationEnum::ELMORAD) CGameProcedure::s_pFX->TriggerBundle(s_pPlayer->IDNumber(), -1, FXID_REGEN_ELMORAD, s_pPlayer->IDNumber(), -1);
...
bool CGameProcMain::MsgRecv_UserIn(DataPack* pDataPack, int& iOffset, bool bWithFX)
{
	...
	{
		if(*eNation==NationEnum::KARUS) CGameProcedure::s_pFX->TriggerBundle(iID, -1, FXID_REGEN_KARUS, iID, -1);
		else if(*eNation==NationEnum::ELMORAD) CGameProcedure::s_pFX->TriggerBundle(iID, -1, FXID_REGEN_ELMORAD, iID, -1);
	}
...
bool CGameProcMain::MsgRecv_MyInfo_LevelChange(DataPack* pDataPack, int& iOffset)
{
	...
	if(iID == s_pPlayer->IDNumber())
	{
		...
		{
			if(s_pPlayer->Nation()==NationEnum::KARUS) CGameProcedure::s_pFX->TriggerBundle(iID, -1, FXID_LEVELUP_KARUS, iID, -1);
			else if(s_pPlayer->Nation()==NationEnum::ELMORAD) CGameProcedure::s_pFX->TriggerBundle(iID, -1, FXID_LEVELUP_ELMORAD, iID, -1);
		}
	}
	else // 다른 넘이다..
	{
		...
		if(pUPC)
		{
			...
			{
				if(pUPC->Nation()==NationEnum::KARUS) CGameProcedure::s_pFX->TriggerBundle(iID, -1, FXID_LEVELUP_KARUS, iID, -1);
				else if(pUPC->Nation()==NationEnum::ELMORAD) CGameProcedure::s_pFX->TriggerBundle(iID, -1, FXID_LEVELUP_ELMORAD, iID, -1);
			}
			pUPC->m_InfoBase.iLevel = iLevel;
		}
	}
...
void CGameProcMain::CommandEnableAttackContinous(bool bEnable, CPlayerBase* pTarget)
{
	...
	if(bEnable)
	{
		...
		if (!ACT_WORLD->canAttackSameNation() && (pTarget->m_InfoBase.eNation == s_pPlayer->m_InfoBase.eNation))
			return;
		if (!ACT_WORLD->canAttackOtherNation() && (s_pPlayer->m_InfoBase.eNation == NationEnum::ELMORAD && pTarget->m_InfoBase.eNation == NationEnum::KARUS))
			return;
		if (!ACT_WORLD->canAttackOtherNation() && (s_pPlayer->m_InfoBase.eNation == NationEnum::KARUS && pTarget->m_InfoBase.eNation == NationEnum::ELMORAD))
			return;
		*/
		//-------------------------------------------------------------------------
	}
