...
bool CPlayerMySelf::IsAttackableTarget(CPlayerBase* pTarget, bool bMesureAngle)
{
	...
	if (!ACT_WORLD->canAttackSameNation() && (pTarget->m_InfoBase.eNation==m_InfoBase.eNation))
		return false;
	if (!ACT_WORLD->canAttackOtherNation() && (m_InfoBase.eNation == NationEnum::ELMORAD && pTarget->m_InfoBase.eNation == NationEnum::KARUS))
		return false;
	if (!ACT_WORLD->canAttackOtherNation() && (m_InfoBase.eNation == NationEnum::KARUS && pTarget->m_InfoBase.eNation == NationEnum::ELMORAD))
		return false;
	*/
	//-------------------------------------------------------------------------
...
bool CPlayerMySelf::CheckCollision()
{
	...
	for(; it != itEnd; it++)
	{
		...
		if (!ACT_WORLD->canAttackSameNation() && (pUPC->m_InfoBase.eNation == m_InfoBase.eNation))
			continue;
		if (!ACT_WORLD->canAttackOtherNation() && (m_InfoBase.eNation == NationEnum::ELMORAD && pUPC->m_InfoBase.eNation == NationEnum::KARUS))
			continue;
		if (!ACT_WORLD->canAttackOtherNation() && (m_InfoBase.eNation == NationEnum::KARUS && pUPC->m_InfoBase.eNation == NationEnum::ELMORAD))
			continue;
		*/
		//-------------------------------------------------------------------------
...
//	if(pZoneInfo && pZoneInfo->bNPCCollisionCheck) //this_zone
	...
	for(; it_N != it_NEnd; it_N++)
	{
		...
		if(*m_InfoBase.eNation == NationEnum::KARUS && *pNPC->m_InfoBase.eNation != NationEnum::ELMORAD) continue; // 적국 엔피씨는 충돌 체크를 한다.
		if(*m_InfoBase.eNation == NationEnum::ELMORAD &&* pNPC->m_InfoBase.eNation != NationEnum::KARUS) continue; // 
		...
		if (!ACT_WORLD->canAttackSameNation() && (pNPC->m_InfoBase.eNation == m_InfoBase.eNation))
			continue;
		if (!ACT_WORLD->canAttackOtherNation() && (m_InfoBase.eNation == NationEnum::ELMORAD && pNPC->m_InfoBase.eNation == NationEnum::KARUS))
			continue;
		if (!ACT_WORLD->canAttackOtherNation() && (m_InfoBase.eNation == NationEnum::KARUS && pNPC->m_InfoBase.eNation == NationEnum::ELMORAD))
			continue;
		*/
		//-------------------------------------------------------------------------
