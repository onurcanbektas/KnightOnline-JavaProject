...
void CGameProcLogIn::MsgRecv_GameServerGroupList(DataPack* pDataPack, int& iOffset)
{
	...
	for(int i = 0; i < iServerCount; i++)
	{
		...
		int freeLimit = CAPISocket::Parse_GetShort(pDataPack->m_pData, iOffset);
		GSI.freeLimit = freeLimit;
		
		int premiumLimit = CAPISocket::Parse_GetShort(pDataPack->m_pData, iOffset);
		GSI.premiumLimit = premiumLimit;
...
void CGameProcLogIn::MsgRecv_AccountLogIn(int iCmd, DataPack* pDataPack, int& iOffset)
{
	...
	if(1 == iResult) // 접속 성공..
	{
		...
		CAPISocket::Parse_GetString(pDataPack->m_pData, iOffset, premiumType, iLen);

		//TODO handle premium (maybe show it in the client?) - maor note
